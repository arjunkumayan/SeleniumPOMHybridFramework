 ...
 * 
 * @Override public void onTestFailure(ITestResult result) { BasePage basePage =
 * new BasePage(); Calendar calendar = Calendar.getInstance(); SimpleDateFormat
 * formater = new SimpleDateFormat("dd_MM_yyyy_hh_mm_ss"); String methodName =
 * result.getName(); if(!result.isSuccess()){ File scrFile =
 * ((TakesScreenshot)basePage.getDriver()).getScreenshotAs(OutputType.FILE); try
